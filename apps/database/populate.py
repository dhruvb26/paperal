...
def process_paper_record(tx, record):
    ...
    for category in categories:
        ...
        if sub_category:
            ...
            """
            tx.run(category_relationship_query, main_category=main_category, sub_category=sub_category)
    ...
    RETURN p
    """
    tx.run(paper_query, id=record['id'], submitter=record['submitter'], title=record['title'],
           comments=record['comments'], journal_ref=record['journal-ref'], doi=record['doi'],
           report_no=record['report-no'], license=record['license'], abstract=record['abstract'],
    ...
    for author in record['authors_parsed']:
        ...
        MERGE (a)-[:AUTHORED]->(p)
        """
        tx.run(paper_author_query, id=record['id'], last_name=author[0], first_name=author[1], middle_name=author[2])
